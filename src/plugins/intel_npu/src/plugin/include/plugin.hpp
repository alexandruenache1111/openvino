...
private:
    // Permanently updates the global log level
    void update_log_level(const ov::AnyMap& properties) const;

    // Returns an RAII guard that applies a per-call LOG_LEVEL for the current thread only,
    ...
    // the persistent baseline or races other threads.
